...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            if input_path.is_file():
                self._convert_file(generator, input_path, output_dir)
            elif input_path.is_dir():
                self._convert_directory(generator, input_path, output_dir)
        except Exception as exc:
            logger.error('HTML to PDF conversion failed: %s', exc)
            raise CommandError(f'Conversion failed: {exc}') from exc

    def _convert_file(self, generator, html_path: Path, output_dir):
        ...
        result_path = generator.generate_pdf(html_path, pdf_path)
    ...
    def _convert_directory(self, generator, input_path: Path, output_dir):
