...
class GetStudentProfileUseCase:
    ...
    def execute(self, student_id: str) -> StudentProfileData:
        participations = (
            self.student_learning_repo.get_profile_participations(student_id)
        )
        work_ids = [
        ...
        return self.analytics_service.build_profile(
            ...
            task_logs=self.student_learning_repo.get_task_logs(student_id),
            work_group_refs=(
                self.student_learning_repo.get_work_group_refs(work_ids)
            ),
        )
