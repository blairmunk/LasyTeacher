...
Актуальный формат банка заданий — `1.4`. Задания, группы и источники переносятся
по UUID, а классификации — по устойчивому естественному ключу кодификатора.
  ...
  "sources": [
    {
      ...
      "name": "Сборник задач по алгебре. 8 класс",
      "short_name": "Алгебра-8",
...
UUID источника следует один раз назначить в массиве `sources` и повторять в
поле `source.id` у заданий из этого источника. При импорте старого файла без
UUID источник сопоставляется по ISBN, краткому или полному названию; валидатор
выводит предупреждение, поскольку такое сопоставление чувствительно к опечаткам.
