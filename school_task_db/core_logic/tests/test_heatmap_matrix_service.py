...
class HeatmapMatrixServiceTests(TestCase):
    def test_aggregates_score_facts_by_student_and_column(self):
        ...
        self.assertIsNone(data.rows[0].cells[1].pct)
        ...
        self.assertIsNone(data.col_averages[1].pct)
        self.assertEqual(data.col_averages[1].css, 'no-data')
    ...
    def test_builds_subtopic_cells_with_subtopic_reference(self):
        ...
        self.assertEqual(data.rows[0].cells[0].column, subtopic)
